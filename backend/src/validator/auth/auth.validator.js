...
export const registerValidator = [
  ...
  body("password")
    .notEmpty().withMessage("validation.passwordRequired")
    .isLength({ min: 8 }).withMessage("validation.passwordMin")
    .trim(),
];

export const loginValidator = [
  body("email").notEmpty().withMessage("validation.emailRequired").isEmail().withMessage("validation.emailInvalid"),
  body("password").notEmpty().withMessage("validation.passwordRequired").trim(),
];
...
export const resetPasswordValidator = [
  body("token").notEmpty().withMessage("validation.tokenRequired"),
  body("password")
    .notEmpty().withMessage("validation.passwordRequired")
    .isLength({ min: 8 }).withMessage("validation.passwordMin")
    .trim(),
...
export const changePasswordValidator = [
  body("currentPassword").notEmpty().withMessage("validation.currentPasswordRequired").trim(),
  body("newPassword")
    .notEmpty().withMessage("validation.newPasswordRequired")
    .isLength({ min: 8 }).withMessage("validation.passwordMin")
    .trim(),
];
